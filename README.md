# Second NodeChain Example Server 

This is an example server for hosting a NodeChain. 

NodeChain properties:  
- consistent API for accessing Nodes (for everyone accessing, but also for easy setup of a new NodeChain) 
- read/write permissions attached to Identity 
- should tie request "cost" to transaction amount, verify transactions that are expensive before running 



...
```
# skip the pruning step on heroku
